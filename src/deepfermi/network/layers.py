...
class Identity(nn.Module):
    ...
    def __init__(self):
        ...
        self.identity = nn.Identity()  # .cuda()
...
class ConvLayer(nn.Module):
    ...
    def __init__(self, shape, nch, nfilters, pad=1, dim=2, **kwargs):
        ...
        if kwargs.get('groups') != None:
            ...
            groups = 1 
        if kwargs.get('padding_mode') != None:
            padding_mode = kwargs.get('padding_mode')
        else:
            padding_mode = 'zeros'            
                   
        if dim == 1:
            self.conv_layer = nn.Conv1d(nch, nfilters, shape, stride=stride, padding=pad, bias=bias, groups=groups, padding_mode=padding_mode)
        elif dim == 2:
            self.conv_layer = nn.Conv2d(nch, nfilters, shape, stride=stride, padding=pad, bias=bias, groups=groups, padding_mode=padding_mode)
        elif dim == 3:
            self.conv_layer = nn.Conv3d(nch, nfilters, shape, stride=stride, padding=pad, bias=bias, groups=groups, padding_mode=padding_mode)
...
class TransposeConv2DLayer(nn.Module):
    ...
    def __init__(self, shape, nch, nout, stride, pad=1):
        ...
        self.tran_conv_layer = nn.ConvTranspose2d(nch, nout, shape, stride=stride, padding=pad, output_padding=[1, 0], bias=False)  # .cuda()
...
class Activation(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get('act_type') == None:
            act_type = 'LeakyReLU'
            kwargs['neg_slope'] = 0.01
            kwargs['inplace'] = True  # False#
        else:
            act_type = kwargs['act_type']

        if kwargs.get('inplace') == None:
            kwargs['inplace'] = True  # False#

        if act_type == 'LeakyReLU':
            if kwargs.get('neg_slope') == None:
                kwargs['neg_slope'] = 0.01
            if kwargs.get('inplace') == None:
                kwargs['inplace'] = True  # False#
...
class Dropout(nn.Module):
    ...
    def __init__(self, dim=2, **kwargs):
        ...
        if dim == 1:
            ...
            self.dropout = nn.Dropout2d(p=dropout, inplace=inplace_boolean) # Dropout2d
        elif dim == 3:
...
class Pooling(nn.Module):
    ...
    def __init__(self, dim=2, pooling_type='Max', **kwargs):
        ...
        if kwargs.get('kernel_size') == None:
...
class Upsampling(nn.Module):
    ...
    def __call__(self, x_usmpl, size):
        ...
        y_usmpl = nn.functional.interpolate(x_usmpl, size=size[2:], mode=self.upsamp_type, align_corners=False)
...
class BatchNorm(nn.Module):
    ...
    def __init__(self, nch, dim=2, trainable=False):
        ...
        if dim == 1:
            self.batch_norm = nn.BatchNorm1d(nch, affine=trainable, track_running_stats=False)
        elif dim == 2:
            self.batch_norm = nn.BatchNorm2d(nch, affine=trainable, track_running_stats=False)
        elif dim == 3:
            self.batch_norm = nn.BatchNorm3d(nch, affine=trainable, track_running_stats=False)
...
class LayerNorm2D(nn.Module):
    ...
    def __call__(self, x_ln):
        ...
        if not hasattr(self, 'layer_norm'):
            self.layer_norm = nn.LayerNorm(inp_shape, elementwise_affine=self.trainable)  # .cuda()
...
class InstanceNorm(nn.Module):
    ...
    def __init__(self, num_features, dim=2, affine=False):
        ...
        if dim==2:
            self.instance_norm = torch.nn.InstanceNorm2d(num_features, affine=affine)
        elif dim==3:
            self.instance_norm = torch.nn.InstanceNorm3d(num_features, affine=affine)        
...
class PixelWiseConditionalInstanceNorm(nn.Module):
    ...
    def __init__(self, num_channels, dim=2, affine=False, cond_disable=False, res_connect=False):
        super(PixelWiseConditionalInstanceNorm, self).__init__()
        if cond_disable==False:
            self.instance_norm_wgt = InstanceNorm(num_channels, dim=dim, affine=affine)
        else:
            self.instance_norm_wgt = ConstantLayer(1)
        self.instance_norm_inp = InstanceNorm(num_channels, dim=dim, affine=affine)
...
class Flattening(nn.Module):
    ...
    def __init__(self, start_dim=1):
        ...
        if (torch_ver_splt[0] >= '1') and (torch_ver_splt[1] >= '1') and (torch_ver_splt[2] >= '6'):
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, shape, nch, nfilters, nconvs, dim=2, ncout=None, pad=1, bnorm=False, res_connect=False, **kwargs):
        ...
        self.res_connect = res_connect
        if self.res_connect:
            self.res_conv = ConvLayer(dim=dim, shape=1, nch=nch, nfilters=ncout, pad=0, **kwargs)

        self.conv_block = []
        for i in range(nconvs - 1):
            self.conv_block.append(ConvLayer(shape, nch, nfilters, dim=dim, pad=pad, **kwargs))
            self.conv_block.append(Activation(**kwargs))
            if bnorm == True:
                self.conv_block.append(BatchNorm(nfilters, dim=dim))
            nch = nfilters
        self.conv_block.append(ConvLayer(shape, nch, ncout, dim=dim, pad=pad, **kwargs))
        self.conv_block.append(Activation(**kwargs))
        if bnorm == True:
