...
def Save(save_path):
    ...
    def save_func(save_obj, save_obj_type, save_obj_name, **kwargs):
        ...
        if save_obj_type == 'model':
            if kwargs.get('sub_folder') == None:
                torch.save(save_obj.state_dict(), Path.joinpath(save_data_dir, save_obj_name))
            else:
                folder = kwargs['sub_folder']
                sub_folder = Path.joinpath(save_data_dir, folder)
                Path(sub_folder).mkdir(parents=True, exist_ok=True)
                torch.save(save_obj.state_dict(), Path.joinpath(sub_folder, save_obj_name))
            ...
            save_obj.savefig(Path.joinpath(save_data_dir, save_obj_name), dpi=500)
            ...
            with open(Path.joinpath(save_data_dir, save_obj_name),'w') as file:
            ...
            if kwargs.get('mode') == None:
                ...
                mode = kwargs['mode']
            with open(Path.joinpath(save_data_dir, save_obj_name), mode) as file:
                file.write(save_obj)

        elif save_obj_type == 'gif':

            if kwargs.get('clim') == None:
                clim = [0,8]
            else:
                clim = kwargs['clim']
            ...
            for n_image in range(save_obj.shape[2]):
                ...
                gif_name =  str.zfill(gif_name, int(np.floor(np.log10(save_obj.shape[2])+1)))
            ...
            imageio.mimsave(Path.joinpath(save_data_dir, save_obj_name), images)
