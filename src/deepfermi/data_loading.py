...
class DatasetDCEPerfusion(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return self.im_sig[idx], self.ctc[idx], self.aif[idx], self.time[idx], self.wlen[idx], self.seg[idx], self.mbolus[idx], self.eta_pretrain[idx], self.mask_od[idx]
    ...
    @classmethod
    def construct_from_npz(cls, file_path, transform, pid_to_load, config, aug_dataset_flag=False, od_enable = True, device='cuda'):
        ...
        for pid in data_dic.keys():
            ...
            ctc = torch.tensor(data_dic[pid]['ctc'], dtype=dtype)
        ...
        eta_pretrain, mask_od, aif, _ = DatasetDCEPerfusion._calc_lbfgs_estimates(pid, ctc, aif, mbolus, seg, time, wlen, osamp, od_enable=od_enable, basal_slc_id=3)
    ...
    def update_precond_bkg_ref(dataset, config, aug_dataset_flag=False, device='cuda'):
        ...
        # Precondition ctc background
        dataset.ctc = DatasetDCEPerfusion._precond_bkg(aif, ctc, seg, time, wlen, osamp, eta_bkg_ref, device)
        # Precondition eta_pretrain background
        dataset.eta_pretrain = DatasetDCEPerfusion._precond_eta_pretrain(eta_pretrain, seg, eta_bkg_ref)
        # Augumented dataset generation        
        if aug_dataset_flag==True:
    ...
    @staticmethod
    def _read_from_npz(file_path, pid_to_load=None):
    ...
    def _stack_myo_slices(pid, im_sig, ctc, aif, time, wlen, seg):
        
        # Stack the slices
        im_sig = rearrange(im_sig, 'npat nt nx ny nslc -> (npat nslc) nx ny nt')
        ctc = rearrange(ctc, 'npat nt nx ny nslc -> (npat nslc) nx ny nt')
        aif = rearrange(aif, 'npat nt nslc -> (npat nslc) nt')
        wlen = rearrange(wlen, 'npat nslc -> (npat nslc)')
        seg = rearrange(seg, 'npat nx ny nslc -> (npat nslc) nx ny')
    ...
    def _precond_bkg(aif, ctc, seg, time, wlen, osamp, eta_bkg_ref, device='cuda'):
        ...
        S = 10
        S_op = rearrange(torch.tensor([1,1/S,S], device=device), 'np -> 1 np 1 1')
        
        # Initializing background perfusion parameters
        flow_bkg = eta_bkg_ref[0]
        delay_bkg = eta_bkg_ref[1]
        decay_bkg = eta_bkg_ref[2]
        eta_bkg = S_op * rearrange(torch.tensor([flow_bkg, delay_bkg, decay_bkg], device=device), 'np -> 1 np 1 1')
        
        for i in range(ctc.shape[0]):
        
            # Extracting slice aif, ctc and segmentation
            wlen_bkg = wlen[i:i+1]
            aif_bkg = torch.tensor(aif[i:i+1, ...,0:wlen_bkg].unsqueeze(1).unsqueeze(1), device=device)
            ctc_full = ctc[i:i+1, ...,0:wlen_bkg]
            seg_bkg = seg[i:i+1]        
            time_bkg = torch.tensor(time[i:i+1, 0:wlen_bkg]/S, device=device)        
            ...
            aif_bkg = Interp_Linear_1D(aif_bkg, size=osamp*aif_bkg.shape[-1])
            time_bkg = Interp_Linear_1D(time_bkg.unsqueeze(0), size=osamp*time_bkg.shape[-1])[0]
    ...
    def _calc_mbolus(im_sig, seg):
        # Calculating the main bolus to be used for outlier injection during training
        ...
        for i in range(0,im_sig.shape[0]):        
            ...
            fill = (1, 1, 1)    
            mask_i = torch.tensor(cv2.fillPoly(seg_i.copy(), contours, fill), dtype=im_sig_i.dtype)
            lv_mask = (mask_i - seg_i).to(torch.float).unsqueeze(-1)
            lv_sig = (lv_mask * im_sig_i)
            max_index = (lv_sig.mean(-1)==torch.max(lv_sig.mean(-1))).nonzero()
    ...
    def _aug_dataset(pid, ctc, aif, mbolus, seg, time, wlen, osamp, flow_init = 0.05, delay_init = 2, decay_init = 0.1, device = 'cuda'):
        ...
        seg = seg.to(device)
        time = time.to(device)
        mb, xdim, ydim, tdim = ctc.shape[0], ctc.shape[1], ctc.shape[2], ctc.shape[3]
        eta_dim = [mb, mb, 3, xdim, ydim]
        eta_aug = seg.unsqueeze(1).unsqueeze(1) * torch.ones(eta_dim, device=device)
        ...
        for i in pbar_outer:
            ...
            for j in pbar_inner:
                ...
                with torch.no_grad():
                    ...
                    eta_aug[i,j,...], mask_od[i,j,0:wlen_i] = fermi_lbfgs_solver(eta_init, ctc_i, aif_ij_2D, seg_i, time_i)
                    ...
                    # plt.plot(time_i.cpu(), mbolus_i.squeeze(0).cpu(), label="mbolus", linewidth=1, color="blue", linestyle="solid")
                    # plt.plot(time_i.cpu(), aif_ij.squeeze(0).cpu(), label="aif_shifted", linewidth=1, color="red", linestyle="solid")
    ...
    def  _precond_eta_pretrain_aug(eta, seg, eta_bkg_ref):        
        # Change the background of the perfusion parameters
        for i in range(seg.shape[0]):    
            # Extracting slice aif, ctc and segmentation
            seg_bkg = seg[i:i+1].unsqueeze(1).repeat(1,eta.shape[1],1,1)
            eta_bkg = eta[i:i+1]
            eta_bkg[:,:,0,...][seg_bkg==0] = eta_bkg_ref[0]
            eta_bkg[:,:,1,...][seg_bkg==0] = eta_bkg_ref[1]
            eta_bkg[:,:,2,...][seg_bkg==0] = eta_bkg_ref[2]
    ...
    def __call__(self, im_sig, ctc, aif, time, seg, eta_gnd, eta_pretrain, mask_od, aug_data_dic):
        ...
        with torch.no_grad():
            for i in range(N_train):
                ...
                im_sig[i] = hfilp(TF.rotate(im_sig[i].moveaxis(-1, -3), angle=rot_angle)).moveaxis(-3, -1)
