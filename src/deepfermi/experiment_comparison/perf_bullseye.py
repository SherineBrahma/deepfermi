...
class PerfusionBullseye():

    def __init__(self, pat_id, plottingSettings, perf_val):
    ...
    def calc_bulls_eye(self,plottingSettings):
        ...
        colorBar = fig.add_axes([0.6, 0.15, 0.01, 0.2])
        ax = plt.subplot2grid((1,1),(0,0),projection='polar')        
        ax.axes.cla()

        # General initialization
        data_avg = self.perf_val
        ...
        cb1 = mpl.colorbar.ColorbarBase(colorBar, cmap=cmap, norm=norm, orientation='vertical')
        cb1.set_label('mean signal intensity')
